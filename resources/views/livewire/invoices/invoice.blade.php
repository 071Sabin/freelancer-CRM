<div class="">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
        ...
        <x-dashboard-card heading="Total Invoices" value="{{ number_format($total_invoices) }}" dataOverTime="All time"
        ...
        <x-dashboard-card heading="Outstanding" value="{{$default_currency->currency->symbol}} 0.00" dataOverTime="Awaiting payment"
        ...
        <x-dashboard-card heading="Overdue" value="{{ number_format($overdueInvoices) }}" dataOverTime="Past due date"
    ...
    <div class="flex flex-col sm:flex-row justify-end items-center my-3 gap-4">
        <flux:modal.trigger name="create-invoice-modal">
            <x-primary-button class="flex gap-1" wire:click="$dispatchTo('invoices.invoice-form-modal', 'reset-form')">
    ...
    {{-- this is invoice form modal triggered to add new invoices, 
    contains view invoice modal, edit invoice modal triggered from data table 
    --}}
